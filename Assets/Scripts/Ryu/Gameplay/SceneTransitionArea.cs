...
public class SceneTransitionArea : MonoBehaviour
{
    ...
    private void TransitionToScene()
    {
        ...
        // 씬 이름을 GameLocation enum으로 변환하여 위치 설정
        if (GameStateManager.Instance != null && !string.IsNullOrEmpty(targetSceneName))
        {
            // 씬 이름이 GameLocation enum 값과 일치하는지 확인
            if (System.Enum.TryParse<GameLocation>(targetSceneName, out GameLocation location))
            {
                GameStateManager.Instance.SetCurrentLocation(location);
                Debug.Log($"[SceneTransitionArea] 위치 설정: {targetSceneName} → {location}");
            }
            else
            {
                Debug.LogWarning($"[SceneTransitionArea] 씬 이름 '{targetSceneName}'을 GameLocation enum으로 변환할 수 없습니다.");
            }
        }
